...
program heterozygosity_ratio
    ...
    ! Window size: 25 KB (±12,5 KB around the SNP)
    integer, parameter :: window_size = 25000
    integer, parameter :: half_window = window_size / 2  ! 12.5 KB
...
10 close(9)
   ...
   ! Process each SNP
   do i = 1, nsnp
      ! Define fixed window around SNP (±12.5 KB)
      left = i
      right = i

      ! Find leftmost SNP within 12.5 KB
      do while (left > 1 .and. pos(i) - pos(left) <= half_window)
          left = left - 1
      end do

      ! Find rightmost SNP within 12.5 KB
      ...
      ! Ignore SNP if less than 50 SNPs in window
      if (total_snps < 50) cycle
